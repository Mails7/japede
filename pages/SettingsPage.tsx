...
import { CogIcon, StorefrontIcon, ChatAltIcon, CreditCardIcon, KeyIcon, BellIcon as NotificationBellIcon, SaveIcon, PlusIcon, TrashIcon, ClockIcon, LinkIcon, EyeIcon as ShowKeyIcon, EyeOffIcon as HideKeyIcon, ClipboardCopyIcon } from '../components/icons';
import { useSettingsContext, defaultAppSettings, defaultStoreSettings, defaultPaymentSettings, defaultWhatsAppSettings, defaultNotificationSettings } from '../contexts/SettingsContext';
import { AppSettings, StoreSettings, PaymentSettings, OpeningHoursEntry, DeliveryFeeType, WhatsAppSettings, NotificationSettings, OpeningHours } from '../types';
import LoadingSpinner from '../components/shared/LoadingSpinner';
import { useAppContext } from '../contexts/AppContext';

type SettingsTab = 'store' | 'payments' | 'whatsapp' | 'notifications' | 'integrations' | 'security'; // Added 'integrations'
...
const SettingsPage: React.FC = () => {
  const { settings, isLoadingSettings, updateSettings } = useSettingsContext();
  const { setAlert } = useAppContext(); 
  const [activeTab, setActiveTab] = useState<SettingsTab>('store');
  const [localSettings, setLocalSettings] = useState<AppSettings>(settings || defaultAppSettings);
  ...
  useEffect(() => {
    if (settings) {
      ...
      if (newlyGeneratedApiKey && settings.n8n_api_key === newlyGeneratedApiKey) {
        // If the saved key matches the one we just generated, keep showing it.
        setShowApiKey(true);
      } else {
        setShowApiKey(false); // Hide by default when settings are loaded
      }
      setNewlyGeneratedApiKey(null); // Reset after processing
    } else {
      setLocalSettings(JSON.parse(JSON.stringify(defaultAppSettings))); // Deep copy
      setShowApiKey(false);
    }
  }, [settings]);
  ...
  const handleStoreInputChange = (field: keyof StoreSettings, value: any) => {
    setLocalSettings(prev => ({
      ...prev,
      store: {
        ...(prev.store || defaultStoreSettings),
        [field]: value,
      },
    }));
  };
  ...
  const handlePaymentInputChange = (field: keyof PaymentSettings, value: any) => {
    setLocalSettings(prev => ({
      ...prev,
      payments: {
        ...(prev.payments || defaultPaymentSettings),
        [field]: value,
      },
    }));
  };
  ...
  const handleOpeningHoursChange = (day: keyof OpeningHours, field: keyof OpeningHoursEntry, value: string | boolean) => {
    setLocalSettings(prev => ({
        ...
        store: {
            ...(prev.store || defaultStoreSettings),
            opening_hours: {
                ...(prev.store?.opening_hours || defaultStoreSettings.opening_hours),
                [day]: {
                    ...(prev.store?.opening_hours?.[day] || defaultStoreSettings.opening_hours[day]),
                    [field]: value,
                }
            }
        }
  ...
  const handleDeliveryFeeChange = (field: keyof StoreSettings['delivery_fee'], value: any) => {
    setLocalSettings(prev => ({
        ...
        store: {
            ...(prev.store || defaultStoreSettings),
            delivery_fee: {
                ...(prev.store?.delivery_fee || defaultStoreSettings.delivery_fee),
                [field]: value,
            }
        }
    }));
  ...
  const handleTopLevelSettingChange = (field: keyof AppSettings, value: any) => {
    setLocalSettings(prev => ({
      ...prev,
      [field]: value,
    }));
  ...
  const generateApiKey = () => {
    ...
    setShowApiKey(true); // Show the newly generated key immediately
  ...
  const copyApiKeyToClipboard = () => {
    if (localSettings.n8n_api_key) {
      navigator.clipboard.writeText(localSettings.n8n_api_key)
        .then(() => setAlert({ message: 'Chave API copiada para a área de transferência!', type: 'success' }))
        .catch(() => setAlert({ message: 'Falha ao copiar a chave API.', type: 'error' }));
    }
  };
  ...
  const handleSaveSettings = async (e: FormEvent) => {
    ...
    if (success) {
      // Alert is handled by updateSettings in context
      // If a new key was generated and saved, it will be reloaded via useEffect,
      // and `showApiKey` logic in useEffect will handle visibility.
    } else {
      // Alert is handled by updateSettings in context
    }
    setIsSaving(false);
    ...
    <form onSubmit={handleSaveSettings} className="space-y-6">
      <div className="flex flex-wrap justify-between items-center gap-y-3">
        <div className="flex items-center space-x-2">
            ...
            <h1 className="text-2xl sm:text-3xl font-semibold text-gray-800">Configurações</h1>
        </div>
        <button
            type="submit"
            disabled={isSaving || isLoadingSettings}
            className="bg-green-500 hover:bg-green-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition duration-150 ease-in-out flex items-center disabled:opacity-70"
        >
            {isSaving ? <LoadingSpinner size="w-5 h-5 mr-2" /> : <SaveIcon className="w-5 h-5 mr-2" />}
            Salvar Alterações
        </button>
      </div>
            ...
            {activeTab === 'store' && (
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label htmlFor="store_name" className="block text-sm font-medium text-gray-700">Nome da Loja*</label>
                            <input type="text" id="store_name" value={localSettings.store.store_name} onChange={e => handleStoreInputChange('store_name', e.target.value)} className="mt-1 block w-full input-style" required />
                        </div>
                        <div>
                            <label htmlFor="phone_number" className="block text-sm font-medium text-gray-700">Telefone Principal*</label>
                            <input type="tel" id="phone_number" value={localSettings.store.phone_number} onChange={e => handleStoreInputChange('phone_number', e.target.value)} className="mt-1 block w-full input-style" placeholder="(XX) XXXXX-XXXX" required />
                        </div>
                    </div>
                    ...
                    <div className="mt-6 pt-4 border-t">
                        ...
                        {DAYS_OF_WEEK.map(day => (
                            <div key={day} className="grid grid-cols-1 sm:grid-cols-4 items-center gap-2 p-2 border rounded-md bg-gray-50">
                                <label htmlFor={`${day}_enabled`} className="col-span-1 sm:col-span-1 flex items-center text-sm font-medium text-gray-700">
                                    <input type="checkbox" id={`${day}_enabled`} checked={localSettings.store.opening_hours[day].enabled} onChange={e => handleOpeningHoursChange(day, 'enabled', e.target.checked)} className="h-4 w-4 text-primary rounded border-gray-300 focus:ring-primary mr-2"/>
                                    {DAY_LABELS[day]}
                                </label>
                    ...
                    <div className="mt-6 pt-4 border-t">
                        ...
                        <div className="space-y-2">
                            {(['fixed', 'per_km', 'free', 'free_above_value'] as DeliveryFeeType[]).map(type => (
                                <label key={type} className="flex items-center space-x-2 p-2 border rounded-md hover:bg-gray-50">
                                    <input type="radio" name="deliveryFeeType" value={type} checked={localSettings.store.delivery_fee.type === type} onChange={() => handleDeliveryFeeChange('type', type)} className="form-radio h-4 w-4 text-primary"/>
                                    <span className="text-sm text-gray-700">
                                        {type === 'fixed' && 'Taxa Fixa'}
                                        {type === 'per_km' && 'Taxa por KM'}
                                        {type === 'free' && 'Entrega Grátis'}
                                        {type === 'free_above_value' && 'Grátis Acima de'}
                                    </span>
                                </label>
                            ))}
                        </div>
                        {localSettings.store.delivery_fee.type === 'fixed' && (
                            <div className="mt-2"><label htmlFor="fixed_amount" className="text-sm">Valor Fixo (R$):</label><input type="number" id="fixed_amount" value={localSettings.store.delivery_fee.fixed_amount || ''} onChange={e => handleDeliveryFeeChange('fixed_amount', parseFloat(e.target.value))} className="ml-2 input-style w-24" step="0.01" /></div>
                        )}
                        {localSettings.store.delivery_fee.type === 'per_km' && (
                            <div className="mt-2"><label htmlFor="amount_per_km" className="text-sm">Valor por KM (R$):</label><input type="number" id="amount_per_km" value={localSettings.store.delivery_fee.amount_per_km || ''} onChange={e => handleDeliveryFeeChange('amount_per_km', parseFloat(e.target.value))} className="ml-2 input-style w-24" step="0.01" /></div>
                        )}
                         {localSettings.store.delivery_fee.type === 'free_above_value' && (
                            <div className="mt-2"><label htmlFor="min_order_for_free_delivery" className="text-sm">Valor Mínimo Pedido p/ Entrega Grátis (R$):</label><input type="number" id="min_order_for_free_delivery" value={localSettings.store.delivery_fee.min_order_for_free_delivery || ''} onChange={e => handleDeliveryFeeChange('min_order_for_free_delivery', parseFloat(e.target.value))} className="ml-2 input-style w-24" step="0.01" /></div>
                        )}
                         <div className="mt-3">
                            <label htmlFor="min_order_value_delivery" className="block text-sm font-medium text-gray-700">Valor Mínimo do Pedido para Entrega (R$)</label>
                            <input type="number" id="min_order_value_delivery" value={localSettings.store.min_order_value_delivery || ''} onChange={e => handleStoreInputChange('min_order_value_delivery', parseFloat(e.target.value))} className="mt-1 input-style w-32" step="0.01" placeholder="Ex: 15.00" />
                        </div>
                    </div>
                </div>
            )}
            {activeTab === 'payments' && (
                 <div className="space-y-6">
                    ...
                    {localSettings.payments.accept_pix && (
                        <div className="pt-4 border-t mt-4">
                            <h3 className="text-md font-medium text-gray-700 mb-1">Configuração PIX</h3>
                             <div><label htmlFor="pix_key_type" className="text-sm">Tipo da Chave PIX:</label>
                                <select id="pix_key_type" value={localSettings.payments.pix_key_type || 'random'} onChange={e => handlePaymentInputChange('pix_key_type', e.target.value)} className="ml-2 input-style">
                                    <option value="cpf">CPF</option><option value="cnpj">CNPJ</option><option value="email">Email</option><option value="phone">Telefone</option><option value="random">Aleatória</option>
                                </select>
                            </div>
                    ...
                    <div className="mt-6 p-4 border border-dashed border-gray-300 rounded-md text-center text-gray-500">
                        Integração com Gateways de Pagamento (Stripe, Mercado Pago, etc.) - Em breve.
                    </div>
                </div>
            )}
             ...
             {activeTab === 'notifications' && ( <div className="p-4 text-gray-600"><h2 className="text-xl font-semibold text-gray-700 mb-2">Notificações</h2> <p>Configurações de alertas sonoros e notificações por email/push (Em desenvolvimento).</p></div>)}
                ...
                <div className="space-y-6">
                    ...
                    <div className="p-4 bg-blue-50 border-l-4 border-blue-400 rounded-md">
                        <p className="text-sm text-blue-700">
                            Use a chave API abaixo para permitir que serviços externos como n8n interajam com seu sistema JáPede.
                            <strong>Guarde esta chave em local seguro. Ela concede acesso para criar pedidos, consultar status, etc.</strong>
                        </p>
                    </div>
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex items-center space-x-2">
                                <input
                                    ...
                                    id="api_key_display"
                                    readOnly
                                    value={localSettings.n8n_api_key}
                                    className="input-style flex-grow"
                                    aria-label="Chave API"
                                />
                                <button type="button" onClick={() => setShowApiKey(!showApiKey)} className="p-2 text-gray-500 hover:text-gray-700" title={showApiKey ? "Esconder Chave" : "Mostrar Chave"}>
                                    {showApiKey ? <HideKeyIcon className="w-5 h-5" /> : <ShowKeyIcon className="w-5 h-5" />}
                                </button>
                                <button type="button" onClick={copyApiKeyToClipboard} className="p-2 text-gray-500 hover:text-blue-600" title="Copiar Chave">
                            ...
                            <button
                                ...
                                className="text-sm text-red-600 hover:text-red-800 font-medium flex items-center py-1 px-2 border border-red-500 rounded hover:bg-red-50 transition-colors"
                            >
                               <TrashIcon className="w-4 h-4 mr-1" /> Revogar Chave API (Requer Salvar)
                            </button>
                    ...
                    >
                        {localSettings.n8n_api_key ? <PlusIcon className="w-5 h-5 mr-2" /> : <KeyIcon className="w-5 h-5 mr-2" />}
                        {localSettings.n8n_api_key ? 'Gerar Nova Chave (Substituir)' : 'Gerar Chave API'}
                    </button>
                    
                    <div className="mt-6 pt-4 border-t">
                        <h3 className="text-md font-semibold text-gray-600 mb-1">Como Usar:</h3>
                        <p className="text-sm text-gray-600">
                            Para integrar com n8n ou outros serviços, inclua esta chave no cabeçalho HTTP <code className="bg-gray-100 p-1 rounded text-xs">Authorization</code> das suas requisições:
                        </p>
      ...
      <style>{`
        ...
        .input-style:focus {
            ...
            box-shadow: 0 0 0 0.2rem rgba(249, 115, 22, 0.25); /* focus:ring-primary focus:border-primary */
        }
